...
gulp.task('compress', function() {
    ...
    gulp.src(sources, {encoding: false}).pipe(tar('shopsuite.tar')).pipe(gulp.dest('dist'));
    return gulp.src(sources, {encoding: false}).pipe(zip('shopsuite.zip')).pipe(gulp.dest('dist'));
});
...
// Copy the bootswatch styles into their own folder so ShopSuite can select one from the collection
...
// Copy the bootstrap style into its own folder so ShopSuite can select it from the collection
...
// /public/resources/shopsuite - contains the minimized files to be packed into shopsuite.min.[css/js]
// /public/resources/[css/js] - contains the unpacked versions to be used in development mode
// /public/resources - contains the packed shopsuite.min.[css/js] and the jquery.min.js

// Copy JavaScript into a folder which will be used as the source to create shopsuite.min.js (except for jquery.mn.js)
...
gulp.task('prod-js', function() {
    ...
    var shopsuite1js = gulp.src(['./node_modules/bootstrap/dist/js/bootstrap.min.js',
    ...
    var shopsuite2js = gulp.src(['./node_modules/bootstrap-daterangepicker/daterangepicker.js',
        './node_modules/js-cookie/src/js.cookie.js',
        './public/js/imgpreview.full.jquery.js',
        './public/js/manage_tables.js',
    ...
    var prod1js = series(shopsuite1js, shopsuite2js).pipe(concat('shopsuite.min.js'))
        .pipe(rev())
...
gulp.task('debug-css', function() {
    var debugcss = gulp.src(['./node_modules/jquery-ui-dist/jquery-ui.css',
        ...
        './public/css/shopsuite_print.css',
        './public/css/shopsuite.css',
...
gulp.task('prod-css', function() {
    var shopsuite1css = gulp.src(['./node_modules/jquery-ui-dist/jquery-ui.min.css',
    ...
    var shopsuite2css = gulp.src(['./node_modules/bootstrap-daterangepicker/daterangepicker.css',
        './node_modules/bootstrap-tagsinput-2021/src/bootstrap-tagsinput.css']).pipe(cleanCSS({compatibility: 'ie8'}));

    var shopsuite3css = gulp.src(['./node_modules/bootstrap-select/dist/css/bootstrap-select.min.css',
    ...
    var shopsuite4css = gulp.src('./node_modules/chartist-plugin-tooltips/dist/chartist-plugin-tooltip.css').pipe(cleanCSS({compatibility: 'ie8'}));

    var shopsuite5css = gulp.src(['./node_modules/chartist-plugin-tooltips/dist/chartist-plugin-tooltip.css',
        './public/css/bootstrap.autocomplete.css',
        './public/css/invoice.css',
        './public/css/shopsuite.css',
        './public/css/shopsuite_print.css',
    ...
    var prodcss = series(shopsuite1css, shopsuite2css, shopsuite3css, shopsuite4css, shopsuite5css)
        .pipe(concat('shopsuite.min.css')).pipe(rev()).pipe(gulp.dest('public/resources'));
