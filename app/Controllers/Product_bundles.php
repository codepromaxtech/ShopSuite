...
class Product_bundles extends Secure_Controller
{
    private Item $item;
    ...
    public function getIndex(): void
    {
        ...
        echo view('product_bundles/manage_modern', $data);
    }
    ...
    public function getView(int $item_kit_id = NEW_ENTRY): void
    {
        ...
        echo view("product_bundles/form_modern", $data);
    }
    ...
    public function postSave(int $item_kit_id = NEW_ENTRY): void
    {
        ...
        if ($this->item_kit->save_value($item_kit_data, $item_kit_id)) {
            ...
            // Handle modern form format (arrays) or old format (associative array)
            $item_ids = $this->request->getPost('item_id');
            $quantities = $this->request->getPost('quantity');
            $sequences = $this->request->getPost('kit_sequence');
            
            // Fallback to old format if modern format not found
            if ($item_ids === null) {
                $item_kit_items_array = $this->request->getPost('item_kit_qty');
                ...
                for ($i = 0; $i < count($item_ids); $i++) {
                    if (!empty($item_ids[$i])) {
                        $item_kit_items[] = [
                            ...
                            'quantity'     => isset($quantities[$i]) ? parse_quantity($quantities[$i]) : 1,
                            'kit_sequence' => isset($sequences[$i]) ? intval($sequences[$i]) : 0
                        ];
                    }
                }
            }
    ...
    /**
     * Checks the validity of the item kit number. Used in app/Views/product_bundles/form.php
     *
     * @return void
