...
class Products extends Secure_Controller
{
    ...
    public function getIndex(): void
    {
        ...
        echo view('products/manage_modern', $data);
    }
    ...
    public function getSearch(): void
    {
        ...
        try {
            ...
            // Get items directly without complex search for now
            $db = \Config\Database::connect();
            $prefix = $db->getPrefix();
            
            $sql = "SELECT i.item_id, i.name, i.item_number, i.category, i.unit_price, 
            ...
            if (!empty($search)) {
                ...
                $sql .= " AND (i.name LIKE '%{$search_safe}%' OR i.item_number LIKE '%{$search_safe}%' OR i.category LIKE '%{$search_safe}%')";
            }
            
            $sql .= " GROUP BY i.item_id ORDER BY {$sort} {$order} LIMIT {$offset}, {$limit}";
            
            $items = $db->query($sql);
            
            // Get total count
            $count_sql = "SELECT COUNT(DISTINCT i.item_id) as count FROM {$prefix}items i WHERE i.deleted = 0";
            if (!empty($search)) {
                $search_safe = $db->escapeString($search);
                $count_sql .= " AND (i.name LIKE '%{$search_safe}%' OR i.item_number LIKE '%{$search_safe}%' OR i.category LIKE '%{$search_safe}%')";
            }
    ...
    public function getStats(): void
    {
        ...
        try {
            ...
            // Total items
            $total_query = $db->query("SELECT COUNT(*) as count FROM {$prefix}items WHERE deleted = 0");
            $total_items = $total_query->getRow()->count ?? 0;
            
            // Low stock items (items below reorder level)
            $low_stock_query = $db->query("SELECT COUNT(DISTINCT i.item_id) as count 
                FROM {$prefix}items i 
                LEFT JOIN {$prefix}item_quantities iq ON i.item_id = iq.item_id 
                WHERE i.deleted = 0 AND COALESCE(iq.quantity, 0) <= i.reorder_level");
            $low_stock = $low_stock_query->getRow()->count ?? 0;
            
            // Total inventory value
            $value_query = $db->query("SELECT SUM(COALESCE(iq.quantity, 0) * i.cost_price) as value 
                FROM {$prefix}items i 
                LEFT JOIN {$prefix}item_quantities iq ON i.item_id = iq.item_id 
                WHERE i.deleted = 0");
            $total_value = $value_query->getRow()->value ?? 0;
            ...
            error_log('[ITEMS] Stats error: ' . $e->getMessage());
            echo json_encode([
                'success' => false,
                'total' => 0,
                'low_stock' => 0,
    ...
    public function getView(int $item_id = NEW_ENTRY): void    // TODO: Long function. Perhaps we need to refactor out some methods.
    {
        ...
        echo view('products/form_modern', $data);
    }
    ...
    public function getInventory(int $item_id = NEW_ENTRY): void
    {
        ...
        echo view('products/form_inventory', $data);
    }
    ...
    public function getCountDetails(int $item_id = NEW_ENTRY): void
    {
        ...
        echo view('products/form_count_details', $data);
    }
    ...
    /**
     * Edit multiple items. Used in app/Views/products/manage.php
     *
     * @return void
     * @noinspection PhpUnused
     */
    public function getBulkEdit(): void
    {
        ...
        echo view('products/form_bulk', $data);
    }
    ...
    /**
     * Generates a template CSV file for item import/update containing headers for current stock locations and attributes. Used in app/Views/products/form_csv_import.php
     *
    ...
    {
        echo view('products/form_csv_import');
    }
