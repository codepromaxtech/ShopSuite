...
-- Indexes for table `shopsuite_cash_up`

ALTER TABLE `shopsuite_cash_up`
  ADD PRIMARY KEY (`cashup_id`),
  ADD KEY `open_employee_id` (`open_employee_id`),
  ADD KEY `close_employee_id` (`close_employee_id`),
  ADD CONSTRAINT `shopsuite_cash_up_ibfk_1` FOREIGN KEY (`open_employee_id`) REFERENCES `shopsuite_employees` (`person_id`),
  ADD CONSTRAINT `shopsuite_cash_up_ibfk_2` FOREIGN KEY (`close_employee_id`) REFERENCES `shopsuite_employees` (`person_id`);

-- AUTO_INCREMENT for table `shopsuite_cash_up`
...
ALTER TABLE shopsuite_cash_up CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;
ALTER TABLE shopsuite_expense_categories CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;
ALTER TABLE shopsuite_expenses CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;
...
UPDATE `shopsuite_expenses`
  INNER JOIN `shopsuite_suppliers`
    ON `shopsuite_expenses`.`supplier_name` = `shopsuite_suppliers`.`company_name`
SET `shopsuite_expenses`.`supplier_id` = `shopsuite_suppliers`.`person_id`;
...
ALTER TABLE `shopsuite_expenses`
  ADD CONSTRAINT `shopsuite_expenses_ibfk_3` FOREIGN KEY (`supplier_id`) REFERENCES `shopsuite_suppliers` (`person_id`);
