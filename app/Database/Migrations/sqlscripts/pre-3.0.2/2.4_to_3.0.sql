...
ALTER TABLE `shopsuite_app_config`
 MODIFY COLUMN `key` varchar(50) NOT NULL,
 MODIFY COLUMN `value` varchar(500) NOT NULL;

UPDATE `shopsuite_app_config` SET `key` = 'receipt_show_total_discount' WHERE `key` = 'show_total_discount';

DELETE FROM `shopsuite_app_config` WHERE `key` = 'use_invoice_template';
DELETE FROM `shopsuite_app_config` WHERE `key` = 'language';
DELETE FROM `shopsuite_app_config` WHERE `key` = 'thousands_separator';
...
UPDATE `shopsuite_modules` SET `sort` = 110 WHERE `name_lang_key` = 'module_config';
...
UPDATE `shopsuite_employees` SET `hash_version` = 1;
