...
UPDATE `shopsuite_sales`
  SET `sale_type` = 0;

UPDATE shopsuite_sales t1
  SET sale_type = 4
WHERE EXISTS (SELECT t2.sale_id FROM shopsuite_sales_items t2 WHERE t1.sale_id = t2.sale_id AND t2.quantity_purchased < 0);
...
DELETE FROM `shopsuite_modules` WHERE `shopsuite_modules`.`module_id` = 'migrate';
DELETE FROM `shopsuite_permissions` WHERE `shopsuite_permissions`.`permission_id` = 'migrate';
DELETE FROM `shopsuite_grants` WHERE `shopsuite_grants`.`permission_id` = 'migrate' AND `shopsuite_grants`.`person_id` = 1;


-- Move Office Module to Right side of Modules list

UPDATE `shopsuite_modules`
SET `sort` = 999
...
CREATE TABLE IF NOT EXISTS `shopsuite_expense_categories` (
...
CREATE TABLE IF NOT EXISTS `shopsuite_expenses` (
...
-- Indexes for table `shopsuite_expense_categories`

ALTER TABLE `shopsuite_expense_categories`
  ADD PRIMARY KEY (`expense_category_id`),
  ADD UNIQUE KEY `category_name` (`category_name`);


-- Indexes for table `shopsuite_expenses`

ALTER TABLE `shopsuite_expenses`
  ADD PRIMARY KEY (`expense_id`),
  ADD KEY `expense_category_id` (`expense_category_id`),
  ADD KEY `employee_id` (`employee_id`);


-- AUTO_INCREMENT for table `shopsuite_expense_categories`

ALTER TABLE `shopsuite_expense_categories`
  MODIFY `expense_category_id` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;


-- AUTO_INCREMENT for table `shopsuite_expenses`

ALTER TABLE `shopsuite_expenses`
  MODIFY `expense_id` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;


-- Constraints for table `shopsuite_expenses`

ALTER TABLE `shopsuite_expenses`
  ADD CONSTRAINT `shopsuite_expenses_ibfk_1` FOREIGN KEY (`expense_category_id`) REFERENCES `shopsuite_expense_categories` (`expense_category_id`),
  ADD CONSTRAINT `shopsuite_expenses_ibfk_2` FOREIGN KEY (`employee_id`) REFERENCES `shopsuite_employees` (`person_id`);


-- Add extra columns to `shopsuite_expenses`
...
DELETE FROM `shopsuite_app_config` WHERE `key` = 'barcode_quality';
